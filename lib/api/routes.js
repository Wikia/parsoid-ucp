...
module.exports = function routes(parsoidConfig, processLogger, parsoidOptions, parse) {
	...
	var wt2htmlDuration = metrics && metrics.makeMetric({
		type: 'Histogram',
		name: 'wt2html.total',
		prometheus: {
			name: 'parsoid_wt2html_duration_seconds',
			help: 'Wikitext to HTML conversion duration in seconds',
	...
	var html2wtDuration = metrics && metrics.makeMetric({
		type: 'Histogram',
		name: 'html2wt.total',
		prometheus: {
			name: 'parsoid_html2wt_duration_seconds',
			help: 'HTML to wikitext conversion duration in seconds',
			buckets: [ 0.01, 0.05, 0.1, 0.3, 1, 10 ]
		},
		labels: {
			names: [],
	...
	routes.v3Middle = function(req, res, next) {
		...
		// Fandom change
		res.locals.mobileve = req.query.mobileve && req.query.mobileve !== 'false' ? '1' : '';
		// SITE-940 Remove after full fandom desktop rollout
		res.locals.fandomdesktop = req.query.fandomdesktop && req.query.fandomdesktop !== 'false' ? '1' : '';
		...
		res.locals.envOptions = {
			...
			// Fandom change
			mobileve: res.locals.mobileve,
			// SITE-940 Remove after full fandom desktop rollout
			fandomdesktop: res.locals.fandomdesktop,
	...
	var wt2html = Promise.async(function *(req, res, wt, reuseExpansions) {
		...
		if (metrics) {
			...
			wt2htmlDuration.timing((Date.now() - startTimers.get('wt2html.total')) / 1000);
		}
	});
	...
	var html2wt = Promise.async(function *(req, res, html) {
		...
		if (metrics) {
			html2wtDuration.timing((Date.now() - startTimers.get('html2wt.total')) / 1000);
