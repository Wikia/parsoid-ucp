...
MWParserEnvironment.prototype.switchToConfig = Promise.async(function *(prefix, noCache) {
	...
	const isGamepedia = yield request({
		...
		console.log('parsoid log', JSON.stringify({
			prefix,
			body,
		}));

		return JSON.parse(body).isGamepedia;
	});
	...
	env.conf.wiki = new WikiConfig(parsoidConfig, resultConf, prefix, isGamepedia);
