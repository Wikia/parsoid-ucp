...
var SiteInfoRequest = require('../mw/ApiRequest.js').SiteInfoRequest;
...
MWParserEnvironment.prototype.switchToConfig = Promise.async(function *(prefix, noCache) {
	...
	try {
		siteInfo = yield SiteInfoRequest.promise(env);
	} catch (error) {
		console.log('parsoid log error', JSON.stringify(error));
	}

	// const isGamepedia = yield request({
	// 	method: 'GET',
	// 	uri: `http://${prefix}/wikia.php`,
	// 	qs: {
	...
	env.conf.wiki = new WikiConfig(parsoidConfig, resultConf, prefix, siteInfo);
