...
function WikiConfig(parsoidConfig, resultConf, prefix) {
	...
	console.log(`parsoid log gamepedia ${general.gamepedia} ${general.gamepedia === true} ${general.gamepedia === 'true'}`);
	const isGamepedia = general.gamepedia === 'true';
	...
	if (isGamepedia) {
		filteredExtensions = allNativeExtensions;
	} else {
		filteredExtensions = allNativeExtensions.filter(function(extension) {
			return !(extension.name === 'Gallery');
		});
	}
	mwApiConf.extensions = filteredExtensions;
	...
	for (var j = 0; j < mws.length; j++) {
		...
		this._mwRegexps[mw.name] =
			new RegExp('^(' +
				this.mwAliases[mw.name].map(JSUtils.escapeRegExp).join('|') +
				')$',
				mw['case-sensitive'] === '' ? '' : 'i');
	}

	this.bswRegexp = new RegExp(
		'(?:' + bsws.map(JSUtils.escapeRegExp).join("|") + ')'
	);
